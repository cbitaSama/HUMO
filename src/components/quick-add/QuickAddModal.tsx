...
export function QuickAddModal({ open, onClose, onSaved, editing, forceLocked }: Props) {
  ...
  // Lock está activo INMEDIATAMENTE si forceLocked viene del padre o si la tx tiene recurring_template_id
  ...
  // Detectar el origen del lock (para el banner)
  useEffect(() => {
    if (!open || !editing) {
      setLinkInfo(null)
      return
    }
    if (!isLocked) return  // si no está bloqueada, no hay info que cargar
    ...
    async function check() {
      ...
      if (partRow) {
        ...
        setLinkInfo({
          kind: "group", groupId: partRow.group_id, groupTitle: gTitle,
          participantName: partRow.name, isInitial: false,
        })
      }
                        ...
                        {loadingLink && !linkInfo ? (
                          ...
                          <>
                            ...
                            <p className="text-xs text-zinc-400 mt-0.5">
                              {linkInfo.isInitial
                                ? "Movimiento inicial. Para cambiarlo, eliminá la deuda."
                                : "Movimiento de cobro. Reabrí la deuda para deshacerlo."}
                            </p>
                          </>
                        ) : linkInfo?.kind === "group" ? (
                          <>
                            ...
                            <p className="text-xs text-zinc-400 mt-0.5">
                              {linkInfo.isInitial
                                ...
                                : "Cobro de un participante. Destildalo desde el grupo."}
                            </p>
                          </>
                        ) : (
