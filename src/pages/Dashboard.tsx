...
export function Dashboard({ refreshKey }: { refreshKey: number }) {
  ...
  const stats = useMemo(() => {
    ...
    const expense = expenses.reduce((s, r) => s + Number(r.amount_bs), 0)
    const income = incomes.reduce((s, r) => s + Number(r.amount_bs), 0)

    // Humo: gastos chiquitos
    const humoTxs = expenses.filter(r => Number(r.amount_bs) < humoThreshold)
    ...
    // Por pagador (cuánto gastaron OTROS en mí)
    const payerMap = new Map<string, { name: string; icon: string; value: number }>()
    for (const r of expenses) {
      if (!r.payer || r.payer.is_self) continue
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto md:max-w-5xl md:p-8 space-y-4">
      ...
      <div className="rounded-2xl border border-zinc-800 bg-gradient-to-br from-zinc-900 to-zinc-950 p-6">
        <p className="text-sm text-zinc-500 mb-2 capitalize">Balance · {monthName}</p>
      ...
      {/* Grid responsive: en desktop donut + humómetro lado a lado */}
      <div className="grid md:grid-cols-2 gap-4">

        {/* Donut por categoría */}
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
          <p className="text-sm font-medium text-zinc-300 mb-1">¿En qué se va tu plata?</p>
          <p className="text-xs text-zinc-500 mb-4">Por categoría · este mes</p>
          {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-orange-900/40 bg-gradient-to-br from-orange-950/30 to-zinc-900 p-6">
            ...
            <p className="text-xs text-zinc-500 mb-3">
              Gastos chicos &lt; {formatBs(humoThreshold)}
            </p>
              ...
              <div className="mt-4">
                <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(100, (stats.humoTotal / stats.expense) * 100)}%` }}
                  />
                </div>
                <p className="text-xs text-zinc-500 mt-2">
                  {((stats.humoTotal / stats.expense) * 100).toFixed(0)}% de tus gastos del mes
                </p>
              </div>
            )}
          </div>
          ...
          {stats.byOthers.length > 0 && (
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
              ...
              <p className="text-xs text-zinc-500 mb-4">Este mes</p>
